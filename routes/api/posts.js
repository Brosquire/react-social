...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      //setting our user from our User model and disabling the password from being returned
      const user = await User.findById(req.user.id).select("-password");
      //setting our post from our Post model
      const post = await Post.findById(req.params.id);
      ...
      //adding new comments to the database using unshift and our newComments object
      post.comments.unshift(newComment);

      //saving our new post with comments added
      await post.save();
...
router.delete("/comment/:id/:comment_id", auth, async (req, res) => {
  try {
    ...
    //get the comment from the post using the req.params query
    ...
    //check if user is the user who made the comment
